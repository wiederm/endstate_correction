...
It should be noted, that :code:`perform_endstate_correction()` returns an instance of :code:`AllResults()`, which, depending on the passed protocol, contains results of an FEP, NEQ switching and SMC protocol.
It is also possible to add equilibrium sampling results to the instance of :code:`AllResults()` retrospectively (:code:`r.equ_results`).
